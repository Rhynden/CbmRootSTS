/** @file CbmHodoSetup.cxx
 ** @author Volker Friese <v.friese@gsi.de>
 ** @date 27.05.2013
 **/

// Include class header
#include "CbmHodoSetup.h"
...
CbmHodoSetup* CbmHodoSetup::fgInstance = NULL;
// -------------------------------------------------------------------------



// -----   Constructor   ---------------------------------------------------
CbmHodoSetup::CbmHodoSetup() : CbmStsElement(kSts, kStsSystem),
			     fDigitizer(NULL),
			     fSettings(NULL),
...
CbmStsSensor* CbmHodoSetup::AssignSensor(Int_t address,
...
Int_t CbmHodoSetup::CreateStations() {
...
CbmStsSensor* CbmHodoSetup::DefaultSensor(Int_t address,
                                         TGeoPhysicalNode* node) {
  ...
  assert(shape);
  //Double_t volX = 2. * shape->GetDX();  not used
  //Double_t volY = 2. * shape->GetDY();  not used

  // Default pitch
  Double_t pitchX   = 1.0;
  Double_t pitchY   = 1.0;

  // Number of strips for front (X) and back (Y) sides.
  Int_t nStripsX = 64;
  Int_t nStripsY = 64;

  // Create default sensor type DssdOrtho
...
CbmStsElement* CbmHodoSetup::GetElement(Int_t address, Int_t level) {
...
const char* CbmHodoSetup::GetLevelName(Int_t level) const {

  // --- Catch legacy (setup with stations)
  if ( fIsOld && level == kStsUnit ) return "station";

  switch(level) {
    case kStsSystem: return "hodo"; break;
...
Int_t CbmHodoSetup::GetStationNumber(Int_t address) {

  // In new, unit-based geometries, the station is obtained from the ladder
  CbmStsElement* ladder = CbmHodoSetup::GetElement(address, kStsLadder);
  assert(ladder);
...
// -----   Initialisation   ------------------------------------------------
Bool_t CbmHodoSetup::Init(const char* geoFile, const char* parFile) {
...
// -----   Singleton instance   --------------------------------------------
CbmHodoSetup* CbmHodoSetup::Instance() {
  if ( ! fgInstance ) fgInstance = new CbmHodoSetup();
  return fgInstance;
}
// -------------------------------------------------------------------------
...
void CbmHodoSetup::ListModules() const {
...
Int_t CbmHodoSetup::ModifyStripPitch(Double_t pitch) {
...
Bool_t CbmHodoSetup::ReadGeometry(TGeoManager* geo) {
  ...
  for (Int_t iNode = 0; iNode < cave->GetNdaughters(); iNode++) {
    TString name = cave->GetDaughter(iNode)->GetName();
     if ( name.Contains("HODO", TString::kIgnoreCase) ) {
      sts = cave->GetDaughter(iNode);
      LOG(info) << fName << ": HODO top node is " << sts->GetName();
      break;
    }
  }
  if ( ! sts ) {
    LOG(error) << fName << ": No top HODO node found in geometry!";
    return kFALSE;
  }
  ...
  if ( hasUnit && (! hasStation) ) fIsOld = kFALSE;
      ...
      << "nor stations!";
  if ( fIsOld ) LOG(warn) << GetName() << ": using old geometry (with stations)";
...
Bool_t CbmHodoSetup::ReadGeometry(const char* fileName) {
...
// -----   Set sensor types   ----------------------------------------------
Int_t CbmHodoSetup::ReadSensorParameters(const char* fileName) {
  ...
  while ( kTRUE ) {  // read one line
    ...
    // Case sensor type DssdOrtho
    if ( sType.EqualTo("DssdOrtho", TString::kIgnoreCase) ) {
...
void CbmHodoSetup::SetDefaultSensorParameters(Double_t dInact,
                                             Double_t pitch) {
  assert( ! fIsInitialised );
  assert( dInact >= 0.);
  assert( pitch >= 0. );
...
Int_t CbmHodoSetup::SetModuleParameters(CbmStsDigitizeParameters* par) {
  ...
  return SetModuleParameters(dynRange, threshold, nAdc, tResol, tDead,
                             noise, zeroNoiseRate, fracDeadChan);
}
// -------------------------------------------------------------------------
...
Int_t CbmHodoSetup::SetModuleParameters(Double_t dynRange,
                                       Double_t threshold,
...
Int_t CbmHodoSetup::SetModuleParameters(const char* fileName) {
...
Int_t CbmHodoSetup::SetSensorConditions(CbmStsDigitizeParameters* par) {
...
// -----   Set conditions for all sensors   --------------------------------
Int_t CbmHodoSetup::SetSensorConditions(Double_t vDep, Double_t vBias,
...
Int_t CbmHodoSetup::SetSensorConditions(const char* fileName) {
